...
#Node to disease in the PPI network
r=open('input_data/PPI_node.csv')
r.readline()
node2disease={}
for l in r:
    ws=l.strip().split(',')
    if ws[2] in g2d:
        ...
        node2disease[ws[0]]=set(g2d[ws[3]].split('&'))
r.close()
ks=list(node2disease.keys())

#Calculate the shortest path lengths
with open('input_data/PPI_network.txt') as r:
    edge=[]
    for l in r:
...
num=len(ks)
w=open('SPL_disease_PPI_network.csv','w')
